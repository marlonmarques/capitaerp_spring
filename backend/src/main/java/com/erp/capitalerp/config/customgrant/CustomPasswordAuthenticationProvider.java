package com.erp.capitalerp.config.customgrant;
...
public class CustomPasswordAuthenticationProvider implements AuthenticationProvider {
	...
	public CustomPasswordAuthenticationProvider(OAuth2AuthorizationService authorizationService,
			...
			PasswordEncoder passwordEncoder, EmpresaRepository empresaRepository) {

		Assert.notNull(authorizationService, "authorizationService cannot be null");
		Assert.notNull(tokenGenerator, "TokenGenerator cannot be null");
		Assert.notNull(userDetailsService, "UserDetailsService cannot be null");
		Assert.notNull(passwordEncoder, "PasswordEncoder cannot be null");
		Assert.notNull(empresaRepository, "empresaRepository cannot be null");
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		String tenantIdentifier = customPasswordAuthenticationToken.getTenantIdentifier();

		if (tenantIdentifier != null && !empresaRepository.existsByTenantIdentifier(tenantIdentifier)) {
			throw new OAuth2AuthenticationException("Empresa não encontrada");
		}
