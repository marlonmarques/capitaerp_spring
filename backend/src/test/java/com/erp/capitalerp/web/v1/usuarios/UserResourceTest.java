...
import com.erp.capitalerp.application.usuarios.Factory;
import com.erp.capitalerp.application.usuarios.UserService;
...
public class UserResourceTest {
    ...
    public void findByIdShouldReturnUserDTOWhenIdExists() throws Exception {
        mockMvc.perform(get("/api/v1/users/{id}", existingId)).andExpect(status().isOk())
                .andExpect(jsonPath("$.id").exists()).andExpect(jsonPath("$.firstName").exists());
    }

    @Test
    public void findByIdShouldReturnNotFoundWhenIdDoesNotExist() throws Exception {
        mockMvc.perform(get("/api/v1/users/{id}", nonExistingId)).andExpect(status().isNotFound());
    }
    ...
    public void insertShouldReturnUserDTOCreated() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/users").content(jsonBody).contentType(MediaType.APPLICATION_JSON)
    ...
    public void updateShouldReturnUserDTOWhenIdExists() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/users/{id}", existingId).content(jsonBody).contentType(MediaType.APPLICATION_JSON)
    ...
    public void updateShouldReturnNotFoundWhenIdDoesNotExist() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/users/{id}", nonExistingId).content(jsonBody)
    ...
    public void deleteShouldReturnNoContentWhenIdExists() throws Exception {
        mockMvc.perform(delete("/api/v1/users/{id}", existingId)).andExpect(status().isNoContent());
    }

    @Test
    public void deleteShouldReturnNotFoundWhenIdDoesNotExist() throws Exception {
        mockMvc.perform(delete("/api/v1/users/{id}", nonExistingId)).andExpect(status().isNotFound());
    }
}